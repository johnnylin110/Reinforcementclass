...
def value_iteration(env, gamma=0.9, max_iterations=10 ** 6, eps=10 ** -3):
    ...
    ##### FINISH TODOS HERE #####
    value = np.array([0 for _ in range(num_spaces)])    # Initialize the value function
    probably_and_reward = env.P # this include the transition probability & rewrad function

    for iteration in range(max_iterations):
        Bellman = np.array([[probably_and_reward[state][action][0][2] for action in range(num_actions)] for state in range(num_spaces)])
        for state in range(num_spaces):
            for action in range(num_actions):
                Bellman[state][action] += (gamma * value[probably_and_reward[state][action][0][1]])
